...
const TOKEN =
  '007eJxTYDg4s2H/zvf3SudVbn2twL1t/tHFdnc7xcpm6StO2s+94PZdBQaTlGQTo7TkpMS0xBQT88QUy8RUY+NkSwtzy0TTlCSz5FLBmrSGQEYGXX8lVkYGCATxWRhyEzPzGBgAOIghhA=='; // 필요하지 않으면 null
...
function VideoCall({ isOperator }) {
  ...
  useEffect(() => {
    ...
    const initAgora = async () => {
      ...
      // isOperator가 False인 경우 실행 (기본 유저)
      if (!isOperator) {
    ...
    return () => {
      // clear 함수 실행 시, False 인 경우 실행 (기본 유저)
      if (!isOperator) {
